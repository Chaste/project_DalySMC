'''
	IMPLEMENTATIONS:
		FunctionalCurationExperiment
'''

from experiment import Experiment

# Functional curation library imports
import fc
...
class FunctionalCurationExperiment(Experiment):

	''' Accepts paths (absolute or relative to Chaste installation location) for functional
		curation protocol and CellML model files. '''
	def __init__(self,protoFile,modelFile,outputFolder=None,solver=CvodeSolver()):
		self.protocol = fc.Protocol(protoFile)
		# The exposeAnnotatedVariables flag allows direct modification of model parameters
	...
	''' Parameters MUST be of the form 'namespace:parameter' with relation to the model.
	...
	def simulate(self,parameters=None):
		...
		if parameters != None:
			for key,val in parameters.iteritems():
				# NOTE: May want to check existence of parameters/namespaces for informative failure.
				tokens = key.split(':')
			...
			# Must adjust initial state AFTER setting parameters so that nested simulations
			# do not reset to default parameter values between loops
	...
	def setInputs(self,inputs):
		for name,value in inputs.iteritems():
			# Value.Array expects a numpy array in its constructor
			if isinstance(value,list):
				self.protocol.SetInput(name,V.Array(numpy.array(value)))
			elif isinstance(value,numpy.ndarray):
				self.protocol.SetInput(name,V.Array(value))
			# If not iterable, assumes a single value (Value.Simple constructor will trigger error otherwise)
			else:
